/**
 * Script de test pour l'API Pappers - Version 2
...
async function testSearchCompany(query) {
  console.log(`\x1b[1m\x1b[36m[TEST]\x1b[0m Recherche de l'entreprise: ${query}`);
  try {
...
async function runTests() {
  ...
  // Test avec un SIREN directement (TF1)
  console.log('\x1b[1m\x1b[35m%s\x1b[0m', '=== TEST AVEC UN SIREN ===');
  console.log('');
  const siren = '552081317'; // SIREN de TF1
  await testCompanyDetails(siren);
  await testFinancialData(siren);
  await testCompanyManagement(siren);
  
  // Test avec une recherche par nom
  console.log('\x1b[1m\x1b[35m%s\x1b[0m', '=== TEST AVEC UNE RECHERCHE PAR NOM ===');
  console.log('');
  const searchResults = await testSearchCompany('Clarins');
  
  if (searchResults && searchResults.resultats && searchResults.resultats.length > 0) {
    const siren2 = searchResults.resultats[0].siren;
    await testCompanyDetails(siren2);
    await testFinancialData(siren2);
    await testCompanyManagement(siren2);
  }
