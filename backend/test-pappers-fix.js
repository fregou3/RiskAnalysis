...
async function runTests() {
  ...
  console.log('');
  const searchResults2 = await testSearchCompany('Clarins');
  
  if (searchResults2 && searchResults2.resultats && searchResults2.resultats.length > 0) {
    const siren = searchResults2.resultats[0].siren;
    await testCompanyDetails(siren);
    await testFinancialData(siren);
    await testCompanyManagement(siren);
  }
